...
import {
  ...
  computeSalesReturnLineAmount,
  formatAmount,
  isSalesReturnLineComplete,
...
function SalesReturnLineRow({
  ...
  const lineAmount = computeSalesReturnLineAmount(line);
  const showItemAsText = Boolean(line.isMainScheduleRow || (line.item && !line.productKey));
  const subInvoiceLabel = line.isMainScheduleRow
    ? "Main"
    : line.subInvoiceNo
    ? `Sub ${line.subInvoiceNo}`
    : "";
  ...
  return (
    <MDBox
      sx={{
        ...gridRowSx,
        bgcolor: line.isMainScheduleRow ? "rgba(76, 175, 80, 0.08)" : "rgba(25, 118, 210, 0.06)",
        "&:hover": {
          bgcolor: line.isMainScheduleRow ? "rgba(76, 175, 80, 0.12)" : "rgba(25, 118, 210, 0.08)",
        },
        ...
        {showItemAsText ? (
          <MDBox>
            {subInvoiceLabel ? (
              <MDTypography
                variant="caption"
                sx={{ display: "block", fontSize: "0.625rem", color: "text.secondary", mb: 0.25 }}
              >
                {subInvoiceLabel}
              </MDTypography>
            ) : null}
...
SalesReturnLineRow.defaultProps = {
  productOptions: [],
  readOnly: false,
  errors: {},
  canDelete: true,
...
export default function SalesReturnLinesGrid({
  lines,
  productOptions,
  errors,
  saving,
  ...
  const canAddLine =
    !readOnly &&
    !saving &&
    (lines.length === 0 || lines.every((line) => isSalesReturnLineComplete(line)));
          ...
          <MDBox sx={{ minWidth: "max-content" }}>
            ...
            {!lines?.length ? (
              <MDTypography
                ...
                py={2}
              >
                Select a Sales Invoice to load line items, or click add to create a row.
              </MDTypography>
            ) : (
              lines.map((line, index) => (
                <SalesReturnLineRow
                  ...
                  canDelete
                  canDuplicate={isSalesReturnLineComplete(line)}
                />
              ))
            )}
          </MDBox>
...
SalesReturnLinesGrid.propTypes = {
...
SalesReturnLinesGrid.defaultProps = {
  productOptions: [],
  errors: {},
  saving: false,
  grandTotal: 0,
...
export { syncSalesReturnLineAmount };
