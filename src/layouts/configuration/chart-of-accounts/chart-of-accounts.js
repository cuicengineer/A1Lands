...
function ChartOfAccounts() {
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      const list = (chartOfAccountsApi.unwrapList(response) || [])
        .map(normalizeCoaRow)
  ...
  const handleSaveForm = async () => {
    ...
    try {
      ...
      if (recordId && formSelectedFiles.length > 0) {
  ...
  const columns = useMemo(
    () => [
      ...
      { Header: "S.No", accessor: "sno", align: "center", width: "5%" },
      { Header: "Acct ID", accessor: "acctId", align: "left", width: "9%" },
      { Header: "Acct Name", accessor: "acctName", align: "left", width: "16%" },
      { Header: "Group", accessor: "groupName", align: "left", width: "11%" },
      { Header: "Sub-Group", accessor: "subGroup", align: "left", width: "13%" },
      { Header: "Control Account", accessor: "controlAccount", align: "left", width: "14%" },
    ],
    []
  );
  ...
  const computedRows = useMemo(
    ...
    [tableRows, canCreate, canEdit, canDelete, formDialogOpen, handleEdit, handleClone]
  );
    ...
    <DashboardLayout>
      ...
      >
        <DataTable
          ...
          canSearch
          entriesPerPage={{ defaultValue: 20, entries: [10, 25, 50, 100] }}
          showTotalEntries
