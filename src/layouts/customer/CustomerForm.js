...
export default function CustomerForm({
    ...
    const fetchCustomerCoaOptions = async () => {
      try {
        ...
        if (savedCoaId !== "" && savedCoaId != null) {
          const existing = findCustomerCoaOption(options, savedCoaId);
          if (!existing) {
            try {
        ...
        if (!cancelled) setCustomerCoaOptions(options);
      } catch (error) {
        console.error("Error fetching customer chart of account options:", error);

        if (!cancelled) setCustomerCoaOptions([]);
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      await onSubmit?.({ ...form, code: buildCustomerCode(form.codeAlpha, form.codeNumeric) });
    ...
    <>
      <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg" scroll="paper">
        <DialogTitle sx={{ color: "#344767", pb: 1 }}>
          {isClone ? "Clone Customer" : isEditMode ? "Edit Customer" : "Add New Customer"}
        </DialogTitle>
