...
function getAll(pageNumber = 1, pageSize = 50) {
  ...
  return requestWithPagination("GET", `/api/ExchangeRates?${params}`);
}
...
async function create(data) {
  ...
  return requestWithPagination("POST", "/api/ExchangeRates", payload);
}
...
async function update(id, data) {
  ...
  return requestWithPagination("PUT", `/api/ExchangeRates/${id}`, payload);
}

async function remove(id) {
  const actionBy = await getActionBy();
  const body = { Action: "Delete", ActionBy: actionBy, ActionDate: new Date().toISOString() };
  return requestWithPagination("DELETE", `/api/ExchangeRates/${id}`, body);
}
