...
function getAll(pageNumber = 1, pageSize = 50) {
  ...
  return request("GET", `/api/RevenueRates?${params}`);
}
...
function create(data) {
  ...
  return request("POST", `/api/RevenueRates`, payload);
}
...
function update(id, data) {
  ...
  return request("PUT", `/api/RevenueRates/${id}`, payload);
}

function remove(id) {
  const payload = { Action: "Delete", ActionBy: "admin", ActionDate: new Date().toISOString() };
  return request("DELETE", `/api/RevenueRates/${id}`, payload);
}
